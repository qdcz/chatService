...
module.exports = async function (ctx) {
    try{
        const {uuid,routerName,rootId,parentId,icon,routerFnId} = ctx.request.body;
        if(!(uuid && routerName)) return ctx.body = codeMap('M201');

        const updRoleInfoResult = await ctx.service.adminRouter.updRouterByUuid(uuid,{routerName,rootId,parentId,icon,routerFnId})
